...
def evaluate_single_horizon(data_window, horizon):
	...
	fit = fit_seirc_decay_mcmc(y_all_daily[:n_train], cumulative_anchor, horizon)
	...
	hybrid_all = np.cumsum(hybrid_daily_all)
	hybrid_low_test = np.cumsum(np.concatenate([hybrid_daily_train, np.clip(daily_seirc_all[n_train:] + ci[:, 0], 0.0, None)]))[n_train:]
	hybrid_high_test = np.cumsum(np.concatenate([hybrid_daily_train, np.clip(daily_seirc_all[n_train:] + ci[:, 1], 0.0, None)]))[n_train:]
	...
	return {
		...
		"hybrid_daily_low_test": np.clip(daily_seirc_all[n_train:] + ci[:, 0], 0.0, None),
		"hybrid_daily_high_test": np.clip(daily_seirc_all[n_train:] + ci[:, 1], 0.0, None),
		...
		"seirc_resid_test": y_all_cum[n_train:] - c_seirc_all[n_train:],
		"seirc_resid_all": y_all_cum - c_seirc_all,
		"hybrid_resid_train": y_all_cum[:n_train] - hybrid_all[:n_train],
		"hybrid_resid_test": y_all_cum[n_train:] - hybrid_all[n_train:],
		"hybrid_resid_all": y_all_cum - hybrid_all,
		"metrics_seirc": compute_metrics(y_all_cum[n_train:], c_seirc_all[n_train:]),
		"metrics_hybrid": compute_metrics(y_all_cum[n_train:], hybrid_all[n_train:]),
		"params": {"beta0": fit["beta0"], "decay_rate": fit["decay_rate"], "E0": fit["E0"], "I0": fit["I0"], "R0": fit["R0"], "C0": fit["C0"], "dispersion": fit["dispersion"], "ARIMA_order": arima_order, "mcmc_iter": MCMC_ITER, "mcmc_burn_in": MCMC_BURN_IN, "mcmc_chains": MCMC_CHAINS},
	}
...
def build_tables(results):
	...
	for result in results:
		...
		param_rows.append({"test_horizon_days": horizon, "beta0": params["beta0"], "decay_rate": params["decay_rate"], "E0": params["E0"], "I0": params["I0"], "R0": params["R0"], "C0": params["C0"], "dispersion": params["dispersion"], "ARIMA_order": str(params["ARIMA_order"]), "mcmc_iter": params["mcmc_iter"], "mcmc_burn_in": params["mcmc_burn_in"], "mcmc_chains": params["mcmc_chains"]})
		row1 = {"test_horizon_days": horizon, "model": "SEIRC-decay"}
		row1.update(result["metrics_seirc"])
		metric_rows.append(row1)
		row2 = {"test_horizon_days": horizon, "model": "SEIRC-decay+ARIMA"}
