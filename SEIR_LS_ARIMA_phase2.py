...
OUTPUT_DIR = BASE_DIR / "outputs_phase2"
...
def simulate_seirc_decay(beta0, decay_rate, e0, i0, c0, num_days):
	if num_days <= 0:
...
def seirc_decay_residuals(theta, y_obs):
	beta0, decay_rate, e0, i0, c0 = theta

	if beta0 <= 0.0 or decay_rate < 0.0 or e0 <= 0.0 or i0 <= 0.0 or c0 < 1.0:
		return np.full_like(y_obs, 1e12, dtype=np.float64)

	c_pred = simulate_seirc_decay(beta0, decay_rate, e0, i0, c0, len(y_obs))[:, 4]
	return c_pred - y_obs


def fit_seirc_decay_least_squares(y_train):
	c0_guess = max(float(y_train[0]), 1.0)
	lower = np.array([1e-6, 0.0, 1e-6, 1e-6, 1.0], dtype=np.float64)
	upper = np.array([5.0, 0.2, 1e8, 1e8, 1e7], dtype=np.float64)

	initial_guesses = [
		np.array([0.40, 0.002, 100.0, 50.0, c0_guess], dtype=np.float64),
		np.array([0.60, 0.005, 300.0, 100.0, c0_guess], dtype=np.float64),
		np.array([0.30, 0.010, 800.0, 300.0, c0_guess], dtype=np.float64),
		np.array([1.00, 0.015, 200.0, 80.0, c0_guess], dtype=np.float64),
	]
	...
	beta0, decay_rate, e0, i0, c0 = best.x
	pred_train = simulate_seirc_decay(beta0, decay_rate, e0, i0, c0, len(y_train))[:, 4]
...
def evaluate_single_horizon(data_window, horizon):
	...
	seirc_fit = fit_seirc_decay_least_squares(y_train)
	beta0 = seirc_fit["beta0"]
	decay_rate = seirc_fit["decay_rate"]
	e0 = seirc_fit["E0"]
	i0 = seirc_fit["I0"]
	c0 = seirc_fit["C0"]

	c_seirc_all = simulate_seirc_decay(beta0, decay_rate, e0, i0, c0, n_all)[:, 4]
...
def build_tables(results):
	...
	for result in results:
		...
		row_seirc = {"test_horizon_days": horizon, "model": "SEIRC-decay"}
		row_seirc.update(result["metrics_seirc"])
		metric_rows.append(row_seirc)

		row_hybrid = {"test_horizon_days": horizon, "model": "SEIRC-decay+ARIMA"}
...
def plot_comparison(results, output_prefix="seirc_decay_vs_seirc_decay_arima_h"):
	...
	for result in results:
		...
		fig, axes = plt.subplots(1, 2, figsize=(13.2, 5.6), gridspec_kw={"width_ratios": [2.5, 1.2]})
		ax = axes[0]
		ax_zoom = axes[1]
		...
		ax.plot(dates[n_train - 1 :], hybrid[n_train - 1 :], color="blue", linewidth=1.5, alpha=0.95, label="SEIR+ARIMA (cumsum)")
		...
		ax_zoom.plot(dates_window, y_window, color="#ff4d4d", linestyle="None", marker="*", markersize=6.4, markerfacecolor="#ff4d4d", markeredgewidth=0.6, label="Observed")
		ax_zoom.plot(dates_window, seirc_window, color="#7a7a7a", linestyle="--", linewidth=1.5, label="SEIR Baseline")
		ax_zoom.plot(dates[n_train - 1 :], hybrid[n_train - 1 :], color="blue", linewidth=1.5, label="SEIR+ARIMA (cumsum)")
		ax_zoom.fill_between(dates_test, hybrid_low_test, hybrid_high_test, color="#9ecae1", alpha=0.55, label="95% prediction interval")
		ax_zoom.axvline(split_date, color="black", linestyle=":", linewidth=1.6)

		y_candidates = np.concatenate([np.asarray(y_window, dtype=np.float64), hybrid_low_test, hybrid_high_test, seirc_window, hybrid_window])
...
def summarize_residuals(residuals):
	...
	if n > 7:
		try:
			jarque_bera_pvalue = float(stats.jarque_bera(residuals).pvalue)
		except Exception:
			pass
...
def build_residual_diagnostics_table(results):
	...
	for result in results:
		horizon = result["horizon"]
		for model_name, key_prefix in (("SEIRC-decay", "seirc"), ("SEIRC-decay+ARIMA", "hybrid")):
			for segment in ("train", "test", "all"):
				row = {"test_horizon_days": horizon, "model": model_name, "segment": segment}
				row.update(summarize_residuals(result[f"{key_prefix}_resid_{segment}"]))
				rows.append(row)
...
def plot_residuals_and_diagnostics(results, output_path):
	...
	for row_idx, result in enumerate(results):
		...
		ax_series.plot(dates, seirc_resid_all, color="#7f7f7f", linestyle="--", linewidth=1.8, label="SEIRC-decay residual")
		ax_series.plot(dates, hybrid_resid_all, color="#1f77b4", linewidth=1.8, label="SEIRC-decay+ARIMA residual")
	...
	fig.suptitle("Residual Diagnostics (SEIRC-decay and SEIRC-decay+ARIMA)", fontsize=14, y=0.995)
...
def main():
	...
	print("\nEstimated parameters (beta0, decay_rate, E0, I0, C0):")
