...
END_DATE = "2009-08-31"
DAILY_PLOT_START_DATE = "2009-05-21"
...
RANDOM_SEED = 42
IC_E_PRIOR_SCALE = 0.20
IC_I_PRIOR_SCALE = 0.45
IC_E_PRIOR_SIGMA_SCALE = 0.12
IC_I_PRIOR_SIGMA_SCALE = 0.20
JUDGE1_EPS = 1e-12
JUDGE1_METRIC_NAMES = (
	"MAE",
	"MSE",
...
def simulate_seirc(beta, e0, i0, r0, num_days, c0=0.0):
	y0 = np.array([N - e0 - i0 - r0, e0, i0, r0, float(c0)], dtype=np.float64)
...
def arima_forecast_draws_from_interval(arima_model, horizon, n_samples, seed):
	if arima_model is None:
		return np.zeros((int(n_samples), int(horizon)), dtype=np.float64)
...
def estimate_ic_priors(train_daily):
	...
	return {
		"E": {"value": total * IC_E_PRIOR_SCALE, "mu": total * IC_E_PRIOR_SCALE, "sigma": max(total * IC_E_PRIOR_SIGMA_SCALE, 10.0), "lower": 1e-3, "upper": max(total * 8.0, 80.0)},
		"I": {"value": total * IC_I_PRIOR_SCALE, "mu": total * IC_I_PRIOR_SCALE, "sigma": max(total * IC_I_PRIOR_SIGMA_SCALE, 15.0), "lower": 1e-3, "upper": max(total * 10.0, 120.0)},
...
def plot_basic_reproduction_posterior(r0_samples, horizon):
	apply_plot_style()
	...
	ax.set_title(f"Posterior distribution of basic reproduction number (H={horizon})")
	ax.set_xlabel(r"$\mathcal{R}_0 = \beta / \gamma$")
	ax.set_ylabel("Density")
	ax.grid(True, linestyle=":", alpha=0.35)
	style_plot_legend(ax, loc="upper right", fontsize=PLOT_SMALL_LEGEND_SIZE)
	fig.tight_layout()
	fig.savefig(FIGURES_DIR / f"mcmc_h{horizon}_basic_reproduction_posterior.png", dpi=PLOT_DPI, bbox_inches="tight")
	plt.close(fig)
...
def save_mcmc_diagnostics(fitter, horizon):
	...
	beta_samples = get_parameter_samples(fitter, "beta")
	basic_r0_samples = beta_samples / GAMMA
	summary_df = pd.concat(
		[summary_df, pd.DataFrame([summarize_samples("basic_reproduction_number", basic_r0_samples)])],
		ignore_index=True,
	)
	...
	plot_basic_reproduction_posterior(basic_r0_samples, horizon)
	return summary_df
...
def fit_seirc_mcmc(y_train_daily, cumulative_anchor, horizon):
	...
	return {
		...
		"basic_reproduction_number": float(param_means["basic_reproduction_number"]),
...
def evaluate_single_horizon(data_window, horizon):
	...
	sim = simulate_seirc(mcmc_fit["beta"], mcmc_fit["E0"], mcmc_fit["I0"], mcmc_fit["R0"], n_all, cumulative_anchor)
	c_seirc_all = sim[1:, 4]
	...
	hybrid_daily_low_test, hybrid_daily_high_test, hybrid_low_test, hybrid_high_test = build_joint_hybrid_interval_for_forecast_segment(
		mcmc_fit["posterior_samples"],
		mcmc_fit["posterior_param_names"],
		n_all,
		n_train,
		cumulative_anchor,
		arima_model,
	...
	# ---- Daily-residual ARIMA correction (separate from cumulative residual ARIMA) ----
	# fit ARIMA on daily residuals (observed daily - SEIRC daily)
	...
	# Construct point forecasts from daily-ARIMA correction
	...
	# cumulative series implied by the daily-ARIMA corrected daily series
	...
	# intervals using daily-ARIMA only and joint (MCMC + daily-ARIMA)
	arima_daily_only_daily_low_test, arima_daily_only_daily_high_test, arima_daily_only_low_test, arima_daily_only_high_test = build_arima_only_daily_residual_hybrid_interval(
		daily_seirc_all[n_train:], arima_model_daily, horizon, last_train_cumulative_daily, hybrid_daily_test_from_daily_arima, hybrid_cum_test_from_daily_arima
	)
...
def build_judge_table(results):
	...
	for result in results:
		...
		for target, observed_key, baseline_key, hybrid_key, needs_slice in target_specs:
			...
			metric_by_model = {}
			models_to_eval = [("SEIRC", baseline), ("SEIRC+ARIMA", hybrid)]
			if hybrid_daily_from_daily_pred.size == observed.size:
				models_to_eval.append(("SEIRC+ARIMA_daily", hybrid_daily_from_daily_pred))
			...
			for hybrid_model in ("SEIRC+ARIMA", "SEIRC+ARIMA_daily"):
				...
				for metric_name in JUDGE1_METRIC_NAMES:
					row[f"judge1_{metric_name}"] = error_reduction_pct(
						metric_by_model["SEIRC"]["judge1"][metric_name],
				...
				for metric_name in JUDGE2_METRIC_NAMES:
					row[f"judge2_{metric_name}"] = error_reduction_pct(
						metric_by_model["SEIRC"]["judge2"][metric_name],
...
def build_residual_diagnostics_table(results):
	...
	for result in results:
		n_train = result["n_train"]
		# Cumulative residual diagnostics (existing behavior)
		...
		if observed_cumulative.size == hybrid_cum_all_from_daily_arima.size and hybrid_cum_all_from_daily_arima.size > 0:
			...
			for segment, series_segment in (
				...
				run_tests = segment == "train"
				row = {"test_horizon_days": result["horizon"], "model": "SEIRC+ARIMA_daily", "segment": segment, "residual_target": "cumulative_cases"}
				row.update(summarize_residuals(series_segment, run_ljung_box=run_tests, run_normality_tests=run_tests))
				rows.append(row)

		# Daily residual diagnostics (observed daily - model daily)
		# SEIRC daily residuals
		...
		seirc_daily_all = np.concatenate([seirc_daily_train, seirc_daily_test]) if seirc_daily_train.size + seirc_daily_test.size > 0 else np.array([], dtype=np.float64)
		# Hybrid daily residuals (from hybrid_daily_all)
		...
		# For daily, compute residual = observed_daily - model_daily
		observed_daily = np.asarray(result.get("y_all_daily", np.array([])), dtype=np.float64)
		seirc_daily_resid_all = seirc_daily_all  # already computed as observed - seirc
		...
		hybrid_daily_resid_all = np.concatenate([hybrid_daily_resid_train, hybrid_daily_resid_test]) if hybrid_daily_resid_train.size + hybrid_daily_resid_test.size > 0 else np.array([], dtype=np.float64)
		...
		# Append rows for daily residual diagnostics
		for model_name, series in (("SEIRC", seirc_daily_all), ("SEIRC+ARIMA", hybrid_daily_resid_all), ("SEIRC+ARIMA_daily", hybrid_daily_arima_resid_all)):
			for segment, series_segment in (("train", series[:n_train] if series.size > 0 else np.array([], dtype=np.float64)), ("test", series[n_train:] if series.size > n_train else np.array([], dtype=np.float64)), ("all", series)):
				run_tests = segment == "train"
...
def plot_comparison(results):
	...
	for result in results:
		...
		for scale_name, transform, cumulative_floor, daily_ylabel, cumulative_ylabel in scale_specs:
			...
			daily_plot_start = pd.to_datetime(DAILY_PLOT_START_DATE)
			...
			if "hybrid_daily_train_from_daily_arima" in result and "hybrid_daily_test_from_daily_arima" in result:
				hair_train = np.asarray(result.get("hybrid_daily_train_from_daily_arima", np.array([])), dtype=np.float64)
				hair_test = np.asarray(result.get("hybrid_daily_test_from_daily_arima", np.array([])), dtype=np.float64)
				if hair_train.size + hair_test.size > 0:
					hybrid_daily_all_from_daily_arima = np.concatenate([hair_train, hair_test])
			...
			if "hybrid_cum_train_from_daily_arima" in result and "hybrid_cum_test_from_daily_arima" in result:
				hc_train = np.asarray(result.get("hybrid_cum_train_from_daily_arima", np.array([])), dtype=np.float64)
				hc_test = np.asarray(result.get("hybrid_cum_test_from_daily_arima", np.array([])), dtype=np.float64)
				if hc_train.size + hc_test.size > 0:
					hybrid_cum_all_from_daily_arima = np.concatenate([hc_train, hc_test])
			...
			def save_daily_scheme_plot(scheme_suffix, scheme_label, daily_series, interval_low, interval_high):
				...
				ax_daily.plot(daily_plot_dates, daily_plot_seirc, color=baseline_color, linestyle="--", linewidth=1.9, label="SEIR")
			...
			def save_cumulative_scheme_plot(scheme_suffix, scheme_label, cumulative_series, interval_low, interval_high):
				...
				ax.plot(dates, seirc, color=baseline_color, linestyle="--", linewidth=2.0, label="SEIR")
...
def plot_residuals_and_diagnostics(results, output_path):
	...
	for row_idx, result in enumerate(results):
		...
		ax_series.plot(dates, seirc_resid_all, color="#7f7f7f", linestyle="--", linewidth=1.8, label="SEIRC cumulative residual")
		ax_series.plot(dates, hybrid_resid_all, color="#1f77b4", linewidth=1.8, label="SEIRC+ARIMA cumulative residual")
		...
		[
			{
				...
				"prediction_interval_method": "Prediction plots use separate ARIMA-only 95% intervals for cumulative-residual and daily-residual ARIMA targets; each interval uses a fixed SEIRC trajectory plus ARIMA residual forecast uncertainty",
				...
				"residual_diagnostics_target": "cumulative_cases",
